  ...
  types: {
    ...
    block: (props) => {
      ...
      switch (value.style) {
        case 'small':
          return (
            <p className="text-sm py-0 my-0">{value.children?.[0]?.text}</p>
          )
        case 'very-small':
          return (
            <p className="text-xs py-0 my-0 ">{value.children?.[0]?.text}</p>
          )
        case 'left':
          return (
            <div className="w-1/3 float-left">{value.children?.[0]?.text}</div>
          )
        case 'right':
          return (
            <div className="w-1/3 float-right">{value.children?.[0]?.text}</div>
          )
        default:
...
export function FollowUpBody(props: FollowUpBodyProps) {
  ...
  return (
    <div
      id={post.slug}
      className={`w-full ${styles.portableText} flex flex-col`}
    >
...
const severityDescription = {
  ...
  prevention:
    "Prévention - agir avant que le problème ne se produise ou s'aggrave",
...
function Severity(props: { post: Post }) {
  return (
    <div className="flex">
      <div className="font-bold w-20">{useLabel('Sévérité', 'Severity')}:</div>
      <div className={'px-2' + getSeverityClass(props.post)}>
        {
          severityDescription[
            (props.post.severity || 'important') + useLangSuffix()
          ]
        }
      </div>
    </div>
...
const statusDescription = {
  ...
  rejected_en:
    'Rejected - the administration does not acknowledge this as a problem',
...
function Status(props: { post: Post }) {
  return (
    <div className="flex">
      <div className="font-bold w-20">{useLabel('Statut', 'Status')}:</div>
      <div className={'px-2' + getStatusClass(props.post.status)}>
...
function ProblemDescription(props: { post: Post }) {
  return (
    <ProblemSection content={props.post['problem']}>
      {useLabel('Description du problème', 'Problem Description')}:
    </ProblemSection>
  )
}
...
function ProblemImpact(props: { post: Post }) {
  return (
    <ProblemSection content={props.post['impact']}>
      {useLabel('Impact du problème', 'Problem impact')}:
    </ProblemSection>
  )
}
...
function ProblemRisks(props: { post: Post }) {
  return (
    <ProblemSection content={props.post['risks']}>
      {useLabel('Risques', 'Risks')}:
    </ProblemSection>
  )
}
...
function NextSteps(props: { post: Post }) {
  return (
    <ProblemSection content={props.post['next_steps']}>
      {useLabel('Prochaines démarches', 'Next steps')}:
    </ProblemSection>
  )
}
...
function Questions(props: { post: Post }) {
  return (
    <ProblemSection content={props.post['questions']}>
      {useLabel('Questions', 'Open Questions')}:
    </ProblemSection>
  )
}
...
function ResolutionCriteria(props: { post: Post }) {
  return (
    <ProblemSection content={props.post['resolution_criteria']}>
      {useLabel('Critères de résolution', 'Resolution Criteria')}:
    </ProblemSection>
  )
}
...
function Proposals(props: { post: Post }) {
  return (
    <ProblemSection content={props.post['proposals']}>
      {useLabel('Propositions', 'Proposals')}:
    </ProblemSection>
  )
}
